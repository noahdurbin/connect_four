class Player
  ...
  def place_piece(column)
    ...
    elsif column == "B" && player_check(@board.a_column) == true
      @board.b_column.find do |cell|
        cell.change_state("X")
        @flag += 1
      end
    elsif column == "C" && player_check(@board.a_column) == true
      @board.c_column.find do |cell|
        cell.change_state("X")
        @flag += 1
      end
    elsif column == "D" && player_check(@board.a_column) == true
      @board.d_column.find do |cell|
        cell.change_state("X")
        @flag += 1
      end
    elsif column == "E" && player_check(@board.a_column) == true
      @board.e_column.find do |cell|
        cell.change_state("X")
        @flag += 1
      end
    elsif column == "F" && player_check(@board.a_column) == true
      @board.f_column.find do |cell|
        cell.change_state("X")
        @flag += 1
      end
    elsif column == "G" && player_check(@board.a_column) == true
